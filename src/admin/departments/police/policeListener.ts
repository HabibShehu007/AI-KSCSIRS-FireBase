...
export function joinPoliceRoom(
  ...
  const unsubscribe = listenToComplaints("police", (complaints) => {
    ...
    console.log("📥 Police complaints updated:", sorted);
    onReceive(sorted, fresh);
  });

  return () => {
    console.log("🧹 Stopped listening to Police complaints");
    unsubscribe();
  };
}
