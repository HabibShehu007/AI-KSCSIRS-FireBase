...
export default function ImmigrationPortal() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("complaints-immigration");
  ...
  useEffect(() => {
    const unsubscribe = joinImmigrationRoom((incoming, newest) => {
      const updated = [...incoming].sort(
        (a, b) => Number(b.timestamp || 0) - Number(a.timestamp || 0)
      );

      localStorage.setItem("complaints-immigration", JSON.stringify(updated));
        ...
        {complaints.map((c) => (
          ...
          >
            ...
            <div className="flex justify-between items-center">
              ...
              <Link
                to={`/admin/immigration/complaint/${c.id}`}
      ...
      {showOverlay && newestComplaint && (
        <NewMessageOverlay
          onEngage={() => {
            ...
            navigate(`/admin/immigration/complaint/${newestComplaint.id}`);
          }}
        />
      )}
    </div>
