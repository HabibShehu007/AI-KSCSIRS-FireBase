// src/admin/immigration/immigrationListener.ts
import { listenToComplaints } from "../../../users/message/firebaseListener";
import type { Complaint } from "../../../users/message/firebaseStorage";

export function joinImmigrationRoom(
  onReceive: (complaints: Complaint[], newest?: Complaint) => void
) {
  console.log("📡 Listening to Immigration complaints via Firestore");

  let lastSeenId: string | null = null;

  const unsubscribe = listenToComplaints("immigration", (complaints) => {
    ...
    console.log("📥 Immigration complaints updated:", sorted);
    onReceive(sorted, fresh);
  });

  return () => {
    console.log("🧹 Stopped listening to Immigration complaints");
    unsubscribe();
  };
}
