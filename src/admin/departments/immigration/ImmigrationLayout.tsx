// src/admin/immigration/immigrationLayout.tsx
import { useRef, useState, useEffect } from "react";
import { Outlet, Link, useNavigate } from "react-router-dom";
import { joinImmigrationRoom } from "./immigrationListener";
import {
  FiHome,
  FiSettings,
  FiBellOff,
...
export default function ImmigrationLayout() {
  ...
  const [isMuted, setIsMuted] = useState(() => {
    return localStorage.getItem("immigration-muted") === "true";
  });
  ...
  const toggleMute = () => {
    const next = !isMuted;
    setIsMuted(next);
    localStorage.setItem("immigration-muted", String(next));
  };

  useEffect(() => {
    const unsubscribe = joinImmigrationRoom((complaints, newest) => {
  ...
  const handleEngage = () => {
    ...
    if (newestComplaintId) {
      navigate(`/admin/immigration/complaint/${newestComplaintId}`);
    } else {
      navigate("/admin/immigration/inbox");
    }
  };
  ...
  const NavLinks = () => (
    <nav className="flex-1 p-4 space-y-2">
      ...
      <Link
        to="/admin/immigration/analytics"
        className="flex items-center gap-2 py-2 px-3 rounded hover:bg-[#09203b]"
        onClick={() => setMobileOpen(false)}
      >
