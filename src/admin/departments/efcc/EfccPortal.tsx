...
export default function EfccPortal() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("complaints-efcc");
  ...
  useEffect(() => {
    const unsubscribe = joinEfccRoom((incoming, newest) => {
      const updated = [...incoming].sort(
        (a, b) => Number(b.timestamp || 0) - Number(a.timestamp || 0)
      );

      localStorage.setItem("complaints-efcc", JSON.stringify(updated));
      ...
      {showOverlay && newestComplaint && (
        <NewMessageOverlay
          onEngage={() => {
            ...
            navigate(`/admin/efcc/complaint/${newestComplaint.id}`);
          }}
        />
      )}
    </div>
