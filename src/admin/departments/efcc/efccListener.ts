...
export function joinEfccRoom(
  ...
  const unsubscribe = listenToComplaints("efcc", (complaints) => {
    ...
    console.log("📥 Efcc complaints updated:", sorted);
    onReceive(sorted, fresh);
  });

  return () => {
    console.log("🧹 Stopped listening to Efcc complaints");
    unsubscribe();
  };
}
