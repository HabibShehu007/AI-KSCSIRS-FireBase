...
export default function CivilDefenceComplaintDetails() {
  const { id } = useParams();
  ...
  const updateComplaint = (updates: Partial<Complaint>) => {
    ...
    localStorage.setItem("complaints-civildefence", JSON.stringify(newList));
  ...
  const handleReply = () => {
    ...
    setIsResolved(true);
    setTimeout(() => navigate("/admin/civil-defence/inbox"), 1500);
  };
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-3xl font-extrabold text-[#0a1f44] border-b pb-3 tracking-wide">
        Civil Defence Complaint Details
      </h2>

      <div className="bg-white p-6 rounded shadow-lg space-y-6">
        {/* Complaint Info */}
        <div className="space-y-3 text-gray-800 text-lg">
          ...
          <p>
            ...
            <span className="font-semibold">{complaint.message}</span>
          </p>
          <p>
        ...
        {complaint.voiceNote && (
          <div>
            <span className="font-bold text-[#0a1f44]">Voice Note:</span>
            <audio
              controls
        ...
        {complaint.files.length > 0 && (
          <div>
            <span className="font-bold text-[#0a1f44]">Files:</span>
        ...
        <div className="flex flex-wrap gap-4 mt-6">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 bg-yellow-500 text-white rounded font-bold hover:bg-yellow-600 transition transform hover:scale-105"
          >
            <FiClock className="text-lg" /> Pending
          </button>
          <button
            onClick={() => handleStatusChange("Investigating")}
            className="flex items-center gap-2 px-5 py-2 bg-blue-600 text-white rounded font-bold hover:bg-blue-700 transition transform hover:scale-105"
          >
            <FiSearch className="text-lg" /> Investigating
          </button>
          <button
            onClick={() => handleStatusChange("Resolved")}
            className="flex items-center gap-2 px-5 py-2 bg-green-600 text-white rounded font-bold hover:bg-green-700 transition transform hover:scale-105"
          >
        ...
        {/* Reply Box */}
        <div className="mt-6">
          <label className="block font-bold mb-2 text-[#0a1f44] text-lg">
            Reply
          </label>
          <textarea
            value={reply}
            ...
            rows={4}
            className="w-full border rounded p-3 text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <button
          onClick={handleReply}
          className="bg-blue-700 text-white px-6 py-3 rounded font-bold hover:bg-blue-800 transition transform hover:scale-105 mt-4"
        >
