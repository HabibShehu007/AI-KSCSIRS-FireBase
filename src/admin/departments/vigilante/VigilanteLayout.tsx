...
export default function VigilanteLayout() {
  ...
  const [isMuted, setIsMuted] = useState(() => {
    return localStorage.getItem("vigilante-muted") === "true";
  });
  ...
  const toggleMute = () => {
    const next = !isMuted;
    setIsMuted(next);
    localStorage.setItem("vigilante-muted", String(next));
  };

  useEffect(() => {
    const unsubscribe = joinVigilanteRoom((complaints, newest) => {
      localStorage.setItem("complaints-vigilante", JSON.stringify(complaints));
  ...
  const handleEngage = () => {
    ...
    if (newestComplaintId) {
      navigate(`/admin/vigilante/complaint/${newestComplaintId}`);
    } else {
        ...
        >
          ...
          >
            <div className="flex justify-between items-center p-4 border-b border-gray-700">
              <span className="text-xl font-bold">Vigilante Portal</span>
